<script setup>
    ...
    const listarEmprestimosPendentes = async () => {
    try {
        const { data } = await axios.get('http://localhost:4000/emprestimos/pendentes');
        dados.value = data;
    } catch (error) {
    ...
    <div class="row justify-content-md-center" style="margin-top: 10px;">
      <div class="col col-md-auto">
        <div class="jumbotron">
          ...
          <p class="lead">Listagem de empréstimos pendentes</p>
          <hr class="my-4">
        </div>
      </div>
    </div>
